...
cleanRedwoodData <- function(redwood_df) {
  ...
  redwood_df$result_time <- 
    strptime(x = as.character(redwood_df$result_time), format = "%Y-%m-%d %H:%M:%S")
  
  
  # remove outlying values
  redwood_df <- redwood_df[!(redwood_df$humidity < -5000 | 
                 (redwood_df$humidity > 100) |
                 (redwood_df$humid_temp < 0) | 
                 (redwood_df$hamatop > 200000) | 
                 (redwood_df$hamabot > 30000)), ]
  
  return(redwood_df)
}


cleanMoteData <- function(mote_df){
  colnames(mote_df)[which(colnames(mote_df) == "ID")] <- "nodeid"
